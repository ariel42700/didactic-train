...
soccer_slang_positive <- c("clinical", "maestro", "magician", "engine", "silky",
                           "baller", "wizard", "slick", "dynamic", "classy",
                           "Precision", "Poise", "Artistry", "Masterful", 
                           "craftsman", "visionary", "sportsmanship", 
                           "crisp", "grit", "majestic")
...
soccer_slang_negative <- c("flopper", "diver", "choker", "lazy", "flop", "wasteful",
                            ...
                            "inconsistent", "flat-footed", "stiff", "disjointed", "jaded", "shafted")
...
```{r}
# Identify indices where there are jumps in dates
jump_indices <- which(diff(tweets_df$date) > 1)

# Add the last index to create the end of the last subset
jump_indices <- c(jump_indices, nrow(tweets_df))

# Split the data into subsets based on jump indices
data_subsets <- lapply(seq_along(jump_indices), function(i) {
  start_index <- ifelse(i == 1, 1, jump_indices[i - 1] + 1)
  end_index <- jump_indices[i]
  subset <- tweets_df[start_index:end_index, ]
  return(subset)
})
...
```{r}
# Function to get the words preceding positive words
get_preceding_words <- function(text, positive_word) {
  preceding_words <- str_extract_all(text, paste0("(\\w+\\s+\\w+\\s+)\\b", positive_word, "\\b"))
